...
class ARPOReward(base_agent.EnvironmentInteractingAgent):
  ...
  def step(self, goal: str) -> base_agent.AgentInteractionResult:
    ...
    #    action_prompt,
    #)
    action_output, is_safe, raw_response = self.model.generate_action(
      before_element_list, goal)
...
Action: {{"action_type": "status", "goal_status": "infeasible"}}"""
    ...
    summary, is_safe, raw_response = self.model.generate_summary(
        summary_prompt,
    )
    if is_safe == False:  # pylint: disable=singleton-comparison
      #  is_safe could be None
